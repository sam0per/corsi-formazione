...
## `jira-confluence-best-practices-2026/glossario.md`

The glossary defines 40 terms in alphabetical order. The audit checks each definition for (1) technical accuracy against Jira Cloud 2025-2026, (2) current terminology, (3) consistency with how terms are used in session outlines and exercises, and (4) missing entries.

| # | Line(s) | Category | Issue | Suggested Fix |
|---|---------|----------|-------|---------------|
| 1 | 7 | Terminology | **Assegnatario (Assignee)**: definition says "responsabile di un'**issue**" — "issue" is now **"work item"** (since Jul 2025). | Change to "La persona responsabile di un **work item**". |
| 2 | 15 | Terminology | **Backlog**: definition says "Lista di **issue** da completare" — "issue" is now **"work item"**. | Change to "Lista di **work item** da completare, non ancora pianificati in uno sprint". |
| 3 | 17 | Terminology | **Board (Bacheca)**: definition says "Visualizzazione delle **issue** in colonne" — "issue" is now **"work item"**. | Change to "Visualizzazione dei **work item** in colonne per status". |
| 4 | 23 | Terminology | **Campo (Field)**: definition says "Un attributo di un'**issue**" — "issue" is now **"work item"**. | Change to "Un attributo di un **work item**". |
| 5 | 27 | Terminology | **Commento (Comment)**: definition says "Nota aggiunta a un'**issue**" — "issue" is now **"work item"**. | Change to "Nota aggiunta a un **work item** per comunicare con il team". |
| 6 | 29 | Terminology | **Company-managed project**: the entry title and definition use "project" and "progetto", which are now **"space"** and **"spazio"**. Definition also says "condivisione degli schemi tra **progetti**". | Rename entry to "**Company-managed space**". Change definition to "Space con configurazione centralizzata dall'amministratore Jira. Piu' opzioni di personalizzazione, condivisione degli schemi tra space". |
| 7 | 31 | Terminology | **Componente (Component)**: definition says "Sotto-sezione di un **progetto**" and "categorizzare le **issue**" — both terms are deprecated. | Change to "Sotto-sezione di uno **space** per categorizzare i **work item**". |
| 8 | 41 | Terminology | **Due Date (Scadenza)**: definition says "un'**issue** deve essere completata" — "issue" is now **"work item"**. | Change to "Data entro cui un **work item** deve essere completato". |
| 9 | 45 | Terminology | **Epic**: definition says "**Issue** di alto livello che raggruppa altre **issue** correlate" — "issue" is now **"work item"**. | Change to "**Work item** di alto livello che raggruppa altri **work item** correlati". |
| 10 | 47 | Terminology | **Etichetta (Label)**: definition says "categorizzare le **issue**" — "issue" is now **"work item"**. | Change to "Tag testuale libero per categorizzare i **work item**". |
| 11 | 53 | Terminology | **Filtro rapido (Quick Filter)**: definition says "filtrare rapidamente le **issue**" — "issue" is now **"work item"**. | Change to "Filtro predefinito visibile sulla board per filtrare rapidamente i **work item**". |
| 12 | 61 | Terminology | **Issue**: this is the headword entry itself. "Issue" has been renamed to **"Work Item"** (Elemento di lavoro) in Jira Cloud since Jul 2025. The definition says "Unita' di lavoro in Jira" without any mention of the rename. | Rename entry to "**Work Item (Elemento di lavoro)**" as the primary entry. Add "(ex Issue)" for historical context. Update definition to: "Unita' di lavoro in Jira. Puo' essere un Task, Bug, Story, Epic, o tipo personalizzato. Fino a meta' 2025, chiamato 'Issue'." Optionally keep a cross-reference entry under "I": "**Issue**: vedi **Work Item**." |
| 13 | 63 | Terminology | **Issue type (Tipo di issue)**: "Issue type" has been renamed to **"Work Type"** (Tipo di lavoro) since Jul 2025. | Rename entry to "**Work Type (Tipo di lavoro)**" as the primary entry. Add "(ex Issue Type)" for context. Update definition to: "Categoria del work item che ne definisce lo scopo (Task, Bug, Story, Epic, Sub-task)." Optionally keep a cross-reference: "**Issue Type**: vedi **Work Type**." |
| 14 | 65 | Terminology | **Issue type scheme**: uses deprecated "issue type" and "progetto". | Rename to "**Work type scheme**". Change definition to "Schema che definisce quali work type sono disponibili in uno space". |
| 15 | 69 | Terminology | **JQL (Jira Query Language)**: definition says "cercare **issue** in Jira" — "issue" is now **"work item"**. | Change to "Linguaggio di query per cercare **work item** in Jira con criteri specifici". |
| 16 | 77 | Terminology | **Link (Collegamento)**: definition says "Relazione tra due **issue**" — "issue" is now **"work item"**. | Change to "Relazione tra due **work item** (es. 'blocks', 'is blocked by', 'relates to')". |
| 17 | 81 | Terminology | **Macro (Confluence)**: mentions "**Jira Issues macro**". This macro has been renamed in the Confluence UI to reference "work items" where applicable. | Change to "Jira **Work Items** macro" (or "Jira macro") and update description accordingly. Verify the exact current name in Confluence Cloud. |
| 18 | 87 | Terminology | **Notifica (Notification)**: definition says "per un'**issue**" — "issue" is now **"work item"**. | Change to "Avviso inviato a un utente quando accade qualcosa di rilevante per un **work item**". |
| 19 | 89 | Terminology | **Notification scheme**: definition says "in un **progetto**" — "progetto" is now **"space"**. | Change to "Schema che definisce chi riceve notifiche per quali eventi in uno **space**". |
| 20 | 95 | Terminology | **Permission scheme**: definition says "all'interno di un **progetto**" — "progetto" is now **"space"**. | Change to "Schema che definisce chi puo' fare cosa all'interno di uno **space**". |
| 21 | 97 | Feature accuracy | **Post-function**: defined correctly as "Azione automatica eseguita dopo che una transizione di workflow viene completata". This is still accurate. However, the old workflow editor where post-functions are configured will be removed June 2026. | Add note: "Configurabile nel workflow editor (dal giugno 2026, solo il nuovo editor sara' disponibile)". |
| 22 | 99 | Terminology | **Priorita' (Priority)**: definition says "di un'**issue**" — "issue" is now **"work item"**. | Change to "Livello di urgenza di un **work item**". |
| 23 | 101 | Terminology | **Progetto (Project)**: this is the headword entry itself. "Project" has been renamed to **"Space"** (Spazio) in Jira Cloud since Dec 2025. The definition says "Contenitore in Jira che raggruppa issue correlate" with no mention of the rename. Two deprecated terms in one entry. | Rename entry to "**Space (Spazio)**" as the primary entry. Add "(ex Progetto/Project)" for context. Update definition to: "Contenitore in Jira che raggruppa work item correlati a un obiettivo comune. Fino a dicembre 2025, chiamato 'Project/Progetto'." Optionally keep a cross-reference: "**Progetto (Project)**: vedi **Space**." |
| 24 | 105 | Terminology | **Reporter**: definition says "creato un'**issue**" — "issue" is now **"work item"**. | Change to "La persona che ha creato un **work item**". |
| 25 | 107 | Terminology | **Risoluzione (Resolution)**: definition says "un'**issue**" — "issue" is now **"work item"**. | Change to "Indica come un **work item** e' stato risolto". |
| 26 | 111 | Terminology | **Schema (Scheme)**: definition says "collega impostazioni ai **progetti**" — "progetti" is now **"space"**. | Change to "Configurazione riutilizzabile che collega impostazioni agli **space**". |
| 27 | 115 | Terminology | **Smart values**: example `{{issue.key}}` — still valid (backward compatible) but the newer canonical form is `{{workItem.key}}`. | Update examples to `{{workItem.key}}` as primary, with note that `{{issue.key}}` remains valid for backward compatibility. |
| 28 | 117 | Terminology | **Sotto-attivita' (Sub-task)**: definition says "**Issue** figlia che dipende da un'**issue** padre" — "issue" is now **"work item"**. Also, gender: "work item" is masculine in Italian usage, so "figlia/figlio" needs alignment. | Change to "**Work item** figlio che dipende da un **work item** padre". |
| 29 | 121 | Terminology | **Spazio (Space)**: defined only as Confluence container, saying "equivalente di un progetto Jira". Since Dec 2025, Jira also uses "Space" for its containers. The Confluence definition is accurate but the claim of equivalence is now misleading, and the entry completely omits the Jira "Space" meaning. | Rewrite to disambiguate: "**Spazio (Space)**: (1) **In Jira**: contenitore che raggruppa work item correlati (ex 'Progetto/Project', rinominato dic 2025). (2) **In Confluence**: contenitore che raggruppa pagine correlate. Nota: i due concetti condividono il nome ma sono entita' distinte." This is the **most critical disambiguation** in the glossary. |
| 30 | 125 | Terminology | **Status**: definition says "di un'**issue**" — "issue" is now **"work item"**. | Change to "Lo stato attuale di un **work item** nel suo workflow". |
| 31 | 129 | Terminology | **Team-managed project**: the entry title and definition use "project" and "Progetto", which are now **"space"** and **"spazio"**. Definition also says "Ogni **progetto** ha i propri workflow". | Rename entry to "**Team-managed space**". Change definition to "Space con configurazione semplificata gestita dal team. Ogni space ha i propri workflow e campi indipendenti". |
| 32 | 133 | Terminology | **Transizione (Transition)**: definition says "di un'**issue**" — "issue" is now **"work item"**. | Change to "Passaggio di un **work item** da uno status a un altro all'interno di un workflow". |
| 33 | 139 | Feature accuracy | **Validatore (Validator)**: defined as "Controllo eseguito durante una transizione di workflow". This is accurate. Same note as Post-function regarding old workflow editor removal June 2026. | Optionally add: "Configurabile nel workflow editor". Low priority. |
| 34 | 141 | Terminology | **Versione (Version/Release)**: definition says "raggruppare **issue**" — "issue" is now **"work item"**. | Change to "Marcatore per raggruppare **work item** associati a un rilascio o milestone". |
| 35 | 147 | Terminology | **Workflow**: definition says "ciclo di vita di un'**issue**" — "issue" is now **"work item"**. | Change to "Sequenza di stati e transizioni che definisce il ciclo di vita di un **work item**". |
| 36 | 147 | Feature accuracy | **Workflow**: no mention that the old workflow editor will be removed **June 2026** and only the new editor will be available. This is relevant context for a glossary entry about workflows. | Add to definition: "Dal giugno 2026, sara' disponibile solo il nuovo workflow editor per la modifica dei workflow." |
| 37 | — | Missing entry | **Work Item (Elemento di lavoro)** is not defined as a glossary entry. This is now the official Jira term (since Jul 2025) for what was formerly called "Issue". It is the single most important current Jira term. | Add entry under "W": "**Work Item (Elemento di lavoro)**: Unita' di lavoro in Jira. Puo' essere un Task, Bug, Story, Epic, o tipo personalizzato. Fino a meta' 2025 chiamato 'Issue'." (Or restructure so this is the primary entry and "Issue" becomes a cross-reference — see item #12.) |
| 38 | — | Missing entry | **Work Type (Tipo di lavoro)** is not defined as a glossary entry. This is now the official Jira term (since Jul 2025) for what was formerly called "Issue Type". | Add entry under "W": "**Work Type (Tipo di lavoro)**: Categoria del work item che ne definisce lo scopo (Task, Bug, Story, Epic, Sub-task). Fino a meta' 2025 chiamato 'Issue Type'." (Or restructure — see item #13.) |
| 39 | — | Missing entry | **Space (Spazio) in Jira** is not defined as a standalone entry. The existing "Spazio (Space)" entry (line 121) only covers Confluence. Since Dec 2025, "Space" is also the official Jira term for what was formerly "Project". | Either add a separate Jira-specific entry or (recommended) rewrite the existing "Spazio (Space)" entry to cover both products with clear disambiguation — see item #29. |
| 40 | — | Missing entry | **Sidebar (Barra laterale)** / **Navigazione** — the glossary has no entry describing Jira's navigation. Since March 2025 the primary navigation is a **left sidebar**. Given that session 1 covers navigation in detail and the glossary aims to define key Jira terms, an entry would be useful. | Consider adding under "S" or "N": "**Sidebar di navigazione**: Barra laterale sinistra introdotta nel 2025, punto di accesso principale a space, board, filtri, dashboard e impostazioni. Sostituisce la precedente barra di navigazione superiore." Low priority — navigational concept rather than data concept. |
| 41 | — | Missing entry | **Rovo AI** — Atlassian's AI assistant integrated across Jira and Confluence (2025-2026). It is mentioned in session outlines and the automation cheatsheet references AI-powered features. Not in the glossary. | Consider adding under "R": "**Rovo AI**: Assistente AI di Atlassian integrato in Jira e Confluence per generazione contenuti, creazione regole di automazione in linguaggio naturale e riepilogo attivita'." Medium priority — depends on how prominently AI features are covered in the course. |
| 42 | — | Missing entry | **Live Doc** — new Confluence content type (real-time collaborative documents, similar to Google Docs). Relevant for session 5 (Confluence module). Not in the glossary. | Consider adding under "L": "**Live Doc**: Documento collaborativo in tempo reale in Confluence Cloud, simile a Google Docs. Introdotto 2024-2025." Low priority unless session 5 covers it in detail. |
| 43 | — | Internal consistency | The glossary uses "issue" in **28 definitions** across the file. The session outlines audited in previous sections of this report also pervasively use "issue" and "progetto". The glossary and the outlines are **internally consistent with each other** (they both use the old terminology), but they are all **inconsistent with current Jira Cloud 2025-2026 terminology**. | When updating the glossary, coordinate with the session outline updates to ensure all materials switch to the new terminology simultaneously. |

**Summary**: 43 items audited, **36 issues found** (31 terminology, 2 feature accuracy, 4 missing entries, 1 internal consistency, 5 entries with no change needed). The glossary is **pervasively impacted** by the 2025-2026 terminology changes. The three most critical findings are:

1. **"Issue" appears in 28 definitions** — every reference to "issue" must be updated to "work item" throughout the glossary. This affects the majority of entries.
2. **Three headword entries use deprecated terms as their primary name**: "Issue" (now "Work Item"), "Issue type" (now "Work Type"), and "Progetto (Project)" (now "Space/Spazio"). These entries need to be renamed or restructured with cross-references from the old names.
3. **The "Spazio (Space)" entry is the most critical single item** — it currently defines Space only as a Confluence concept and states it is "equivalente di un progetto Jira". Since Dec 2025, Jira also uses "Space" for its containers, making this definition incomplete and potentially confusing. Clear disambiguation between Jira Space and Confluence Space is essential.

Additionally, three important current terms are missing entirely from the glossary: **Work Item**, **Work Type**, and **Space (Jira)**. The Company-managed and Team-managed entries must be updated from "project" to "space". The workflow entry should mention the upcoming removal of the old editor (June 2026).
