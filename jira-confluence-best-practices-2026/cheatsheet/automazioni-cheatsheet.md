...
|---------|---------------------|---------------|
| **Issue created** | Viene creata una nuova issue | Auto-assegnazione, creazione sotto-task |
| **Issue transitioned** | Cambia lo status | Notifica al reporter, aggiorna campo |
...
| **Issue assigned** | Viene assegnata/riassegnata | Email di notifica al nuovo assegnatario |
...
| **Issue deleted** | Viene eliminata un'issue | Log/audit |

## Condizioni principali

| Condizione | Verifica se... | Esempio |
|-----------|----------------|---------|
| **Issue fields condition** | Un campo ha un certo valore | `Priority = Critical` |
| **JQL condition** | L'issue corrisponde a una query JQL | `labels = "urgente"` |
| **User condition** | L'utente ha un certo ruolo | Solo per admin |
| **Related issues condition** | Le issue collegate soddisfano criteri | Tutte le sotto-task sono "Done" |
...
|--------|---------|---------|
| **Transition issue** | Cambia lo status | Sposta a "In Progress" |
| **Edit issue** | Modifica un campo | Cambia priorità, aggiungi etichetta |
| **Assign issue** | Assegna l'issue | Assegna al team lead |
| **Add comment** | Aggiunge un commento | Messaggio automatico |
| **Send email** | Invia un'email | Notifica al reporter |
| **Create issue** | Crea una nuova issue | Issue di follow-up |
| **Create sub-tasks** | Crea sotto-attività | Checklist onboarding |
| **Link issues** | Collega issue tra loro | Blocca/è bloccata da |
| **Send web request** | Chiama un webhook | Integrazione esterna |
| **Log action** | Registra nel log | Debug/audit |
| **Lookup issues** | Cerca issue con JQL | Per azioni batch |

## Smart values (variabili)

| Smart value | Restituisce | Esempio output |
|------------|-------------|----------------|
| `{{issue.key}}` | Chiave issue | `IT-123` |
| `{{issue.summary}}` | Titolo issue | `Fix stampante` |
...
| `{{issue.url}}` | URL dell'issue | `https://tyvak.atlassian.net/browse/IT-123` |
...
Azione:  Assign issue → IT Manager
```
...
1. **Audit log**: Project settings → Automation → Audit log
...
- Verificare i **limiti mensili** del piano Cloud (Free: 100/mese, Standard: 1700/mese)
- **Testare sempre** su una singola issue prima di attivare su tutto il progetto
