...
Vai su **Project settings → Automation → Create rule** nel progetto di training.
...
- **Condizione**: Issue type = Task
- **Azione**: Assign issue to → Current user (o un utente specifico)
- **Nome regola**: `Auto-assegnazione task`

Testa la regola creando una nuova issue di tipo Task.
...
- **Azione**: Add comment → "⚠️ Attenzione: questo ticket critico è aperto da più di 24h. Richiesta escalation."
...
- **Azione**: Transition issue → "Urgente" (se lo status esiste)
...
- **Trigger**: Issue transitioned → status = "Risolto"
- **Condizione**: Issue type = Task AND labels contains "reclamo"
